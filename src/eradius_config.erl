...
-define(rate_config_options, [limit, max_size, max_time]).
...
validate_new_server_config({Name, {IP, ListOfPorts}}, Nodes) ->
    validate_new_server_config({Name, {IP, ListOfPorts}, []}, Nodes);
validate_new_server_config({Name, {IP, ListOfPorts}, Options}, Nodes) ->
    Validating = [get_app_env(Name), validate_ip(IP), validate_ports(ListOfPorts),
                   map_helper(fun(Option) -> validate_new_server_option(Option) end, Options)],
    if_valids_continue(Validating, fun() -> validate_new_server(Validating, Nodes) end).

validate_new_server([NasList, IP, ListOfPorts, Options], Nodes) ->
    Values = validate_new_nas_list(NasList, {IP, ListOfPorts, Nodes}),
    if_valids_continue(Values, fun() -> lists:map(fun(Port) -> {{IP, Port, Options}, Values} end, ListOfPorts) end).

validate_new_server_option({rate_config, Options}) when is_list(Options) ->
    {rate_config, map_helper(fun({Atom, Integer} = V) when ?pos_int(Integer) ->
                                     case lists:member(Atom, ?rate_config_options) of
                                         true -> V;
                                         false -> ?invalid("false option for rate_config: ~p~n", [V])
                                     end;
                                (V) -> ?invalid("false option for rate config: ~p~n", [V])
                             end, Options)};
validate_new_server_option({rate_config, Option}) when is_atom(Option) ->
    case get_app_env(Option) of
        {invalid, _} = Invalid -> Invalid;
        Values -> validate_new_server_option({rate_config, Values})
    end;
validate_new_server_option(Option) ->
    ?invalid("not supported server option: ~p~n", [Option]).
...
validate_behavior_naslist({Behavior, ListOfNases}, {_IP, _ListOfPorts, Nodes}) when is_list(ListOfNases) ->
    BehaviourNases = [validate_behavior(Behavior), validate_naslist(ListOfNases, Nodes)],
    if_valids_continue(BehaviourNases, fun() -> build_nas_behavior_list(BehaviourNases) end).
...
validate_nas({IP, Secret}, {root, Nodes}) -> validate_nas(validate_ip(IP), Secret, root, Nodes);
validate_nas({IP, Secret, Name}, Nodes)  -> validate_nas(validate_ip(IP), Secret, Name, proplists:get_value(Name, Nodes));
validate_nas(Term, _) -> ?invalid("bad term in NAS specification: ~p", [Term]).

validate_nas({invalid, _} = Invalid, _Secret, _Name, _Nodes) -> Invalid;
validate_nas(IP, Secret, Name, undefined) -> validate_nas(IP, Secret, Name, validate_handler_nodes(Name));
validate_nas(IP, _Secret, Name, {invalid, _}) -> ?invalid("group ~p for nas ~p is undefined", [Name, IP]);
validate_nas(IP, Secret, _Name, Nodes) when ?is_io(Secret) -> {IP, validate_secret(Secret), Nodes};
validate_nas(_IP, Secret, _Name, _) -> ?invalid("bad RADIUS secret: ~p", [Secret]).

% --------------------------------------------------------------------------------------------------
% -- direct validation function

validate_ip(IP) when is_list(IP) ->
    ok_error_helper(inet_parse:ipv4_address(IP), {"bad IP address: ~p", [IP]});
...
build_nas_behavior_list([{Module, Nas, Args}, ListOfNases]) ->
    lists:map(fun({IP, Secret, Nodes}) ->
                      {IP, Secret, Nodes, Module, Args}
                     %% TODO: Decide to use autogenerated NAS Ids
        ...
        [Server | _] ->
            case Server of
                ...
                {_Name, ServerConf} when is_tuple(ServerConf) ->
                    validate_new_config();
                {_Name, ServerConf, _} when is_tuple(ServerConf) ->
                    validate_new_config()
            end;
        [] ->
            validate_server_config(dedup_keys(Config))
...
%% May be use monads later
if_valids_continue(ListOfValues, Action) when is_list(ListOfValues)  ->
    case lists:keyfind(invalid, 1, ListOfValues) of
        false -> Action();
        {invalid, _} = Invalid -> Invalid
    end;
if_valids_continue({invalid, _} = Invalid, _Action) -> Invalid;
if_valids_continue(_Values, Action) -> Action().
