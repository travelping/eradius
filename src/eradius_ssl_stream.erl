...
%% This file is take from the Erlang 15B and modified to work without a
%% connection socket
...
%%--------------------------------------------------------------------
-spec security_parameters(#sslsocket{}) -> {ok, #security_parameters{}}| {error, reason()}.
%% 
%% Description: Deliver transport session data to the ssl connection
%%--------------------------------------------------------------------
security_parameters(#sslsocket{pid = Pid, fd = new_ssl}) ->
    ssl_connection:security_parameters(Pid).

%%--------------------------------------------------------------------
-spec transport_recv(#sslsocket{}, iodata()) -> ok | {error, reason()}.
%% 
%% Description: Deliver transport session data to the ssl connection
%%--------------------------------------------------------------------
...
cipher_suites(What) -> ssl:cipher_suites(What).
