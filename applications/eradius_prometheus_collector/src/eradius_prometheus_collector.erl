...
get_server_labels_and_val(MetricIdx, #nas_counter{} = Cnt) ->
    ...
    {Value, [{server_name, Cnt#nas_counter.server_name}, {server_ip, inet:ntoa(ServerIP)}, {server_port, ServerPort},
             {nas_id, NasId}, {nas_ip, inet:ntoa(NasIP)}]}.
...
get_client_labels_and_val(MetricIdx, #client_counter{} = Cnt) ->
    ...
    Value = element(MetricIdx + 1, Cnt),
    {Value, [{server_name, Cnt#client_counter.server_name}, {server_ip, ServerIP}, {server_port, ServerPort},
